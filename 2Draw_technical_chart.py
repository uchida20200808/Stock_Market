...
def company_stock(start, end, company_code):
    #select a datasorce properly.
    df = data.DataReader(company_code, 'yahoo')
    df = df[(df.index>=start) & (df.index<=end)]
    
    date = df.index
    #check a definition of price.Here is Adj close.
    close = df['Adj Close']
...
#select definitions of start, end.
start = '2019-10-01'
end = '2020-10-01'

#select code_name which you'd like to analyze the trend. 
